...
# fungsi menu cari psikolog online
def menu_cari_psikolog_online():
    while True:
        ...
        print("=== Cari Psikolog Online ===")
        search_input = input("Masukkan nama atau topik psikolog yang ingin dicari: ")
        ...
        if hasil:
            ...
            for i, psikolog in enumerate(hasil, start=1):
                print(f"{i}. Nama: {psikolog['nama']}, Topik: {psikolog['topik']}")
            ...
            if pilihan .isdigit() and 0 < int(pilihan) <= len(hasil):
                ...
                tampilkan_profil_psikolog_online(hasil[int(pilihan)-1])
                break
...
def cari_psikolog(search_input):
    ...
    with open('./database/data_psikolog_online.csv', 'r') as file:
        ...
        for row in csv_reader:
            ...
            if search_input.lower() in row['nama'].lower() or search_input.lower() in row['topik'].lower():
...
def lihat_daftar_psikolog_online():
    ...
    while True:
            ...
            if pilihan .isdigit() and 0 < int(pilihan):
                # menjalankan fungsi untuk menampilkan profil psikolog online yang sesuai dengan pilihan user
                tampilkan_profil_psikolog_online(daftar_psikolog[int(pilihan)-1])
                break
...
# fungsi untuk menampilkan profil psikolog online sesuai dengan pilihan user
def tampilkan_profil_psikolog_online(psikolog):
    while True:
        # variabel untuk mendeteksi sistem operasi
        sistem_operasi = os.name
        ...
        pilihan = input("Masukkan pilihan (1/2): ")
        if pilihan == "1":
            pesan_sesi_konseling_online(psikolog)
        elif pilihan == "2":
            break
...
# fungsi untuk pesan sesi konseling online
def pesan_sesi_konseling_online(psikolog):
    ...
    while True:
        ...
        print("1. Voice Call")
        print("2. Video Call")
        # Pilihan metode konseling online
        pilihan_metode = input("Pilih nomor metode konseling yang diinginkan (1/2): ")
        if pilihan_metode == "1":
            metode_konseling = "Voice Call"
            break
        elif pilihan_metode == "2":
            metode_konseling = "Video Call"
            break
        else:
            print("Pilihan metode konseling tidak valid. Silahkan masukkan pilihan yang tersedia!")
